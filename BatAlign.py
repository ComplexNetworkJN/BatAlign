...
def getPositon(arr):
    raw, column = arr.shape  # get the matrix of a raw and column
    _positon = np.argmax(arr)  # get the index of max in the a
...
def Bat(greedy_align,nnodes1,nnodes2,G1,G2,Population,N_gen,alpha,gamma):
    ...
    for t in range(N_gen):
        for i in range(Population):
            ...
            x_negative = [-2 if ne == -1 or nf == 1 else ne for ne, nf in zip(x_new, v[i])]  # 保留不动的解，其余为0
